    ...
    <div class="w-full px-4 lg:mt-8 sm:px-6 md:px-8 lg:pl-72 z-[0]">
        ...
        <div class="flex mt-8 h-10 justify-between">
            <div class="flex h-full gap-6 w-6/12">
                <form class="w-2/3" action="<?= base_url('/data-manajer') ?>" method="GET">
                    <label for="keyword"
                        class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Search</label>
                    <div class="relative">
                        <input name="keyword" id="search"
                            class="block w-full p-2 ps-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-white "
                            placeholder="Search" autocomplete="off">
                        <button type="submit"
                            class="text-white absolute end-[3px] bottom-[5.5px] font-medium rounded-lg text-sm px-4 py-[5px]"><svg
                                ...
                                <td class="px-4 py-2 pr-6 text-right relative">
                                    <div x-data="{ open: false }" class="relative inline-block text-left z-20" @click.stop>
                                        ...
                                        <div x-show="open" @click.away="open = false" x-transition
                                            ...
                                            <a onclick="openEditPeriodeModal('<?= $p['id'] ?>', '<?= esc($p['tahun']) ?>', '<?= esc($p['bulan']) ?>')"
