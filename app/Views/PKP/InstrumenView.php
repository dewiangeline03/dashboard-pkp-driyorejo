...
<div class="container-sm">
    <div class="w-full px-4 lg:mt-8 sm:px-6 md:px-8 lg:pl-72 z-[0]">
        <h1 class="!text-4xl mb-4 font-semibold text-slate-800">Daftar Instrumen</h1>
        ...
        <div class="flex mt-8 h-10 justify-between">
            <div class="flex h-full gap-6 w-6/12">
                <form class="w-2/3" action="<?= base_url('/data-manajer/instrumen') ?>" method="GET">
                    ...
                    <div class="relative">
                        <input type="text" name="keyword" id="search" value="<?= esc($keyword ?? '') ?>"
                            ...
                            placeholder="Search Nama Instrumen" autocomplete="off">
            ...
            <button type="button"
                ...
                data-hs-overlay="#modal-instrumen-add">
                Tambah
            </button>
        </div>
                ...
                <div class="p-1.5 min-w-full inline-block align-middle">
                    <div class="border rounded-lg overflow-hidden">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 text-start text-sm font-medium text-gray-800">Nama
                                        Instrumen</th>
                                    <th class="py-3 text-center text-sm font-medium text-gray-800">Aksi
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                        ...
                                        <tr>
                                            ...
                                            <td class="py-4 whitespace-nowrap text-center text-sm font-medium">
                                                <div class="flex items-center justify-center gap-6">
                                                    <a onclick="openEditInstrumenModal('<?= $row['id'] ?>', '<?= esc($row['nama']) ?>')"
                                                    ...
                                                    <a onclick="openConfirmModal('<?= $row['id'] ?>')" class="cursor-pointer">
                                                        <span
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="2" class="px-6 py-4 text-center text-gray-500">Belum ada data
                                            instrumen.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
        ...
        <!-- Modal Instrumen Update -->
        <div id="modal-instrumen-update"
                ...
                <div
                    ...
                    <div class="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700">
                        ...
                        <button type="button"
                            ...
                            data-hs-overlay="#modal-instrumen-update" onclick="reset()">
                    ...
                    <div class="p-4 overflow-x-auto">
                        <form id="form-UpdateModal" action="<?= site_url('/data-manajer/instrumen/update') ?>"
                            method="post">
                            <?= csrf_field() ?>
                            <input type="hidden" name="id" id="instrumen-update-id">
                            <div class="mb-4">
                                <label class="block mb-1">Nama Instrumen</label>
                                <input type="text" name="nama" id="instrumen-update-nama"
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700">
                        ...
                        <button type="button"
                            class="flex justify-center items-center w-7 h-7 text-sm font-semibold rounded-full border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                            data-hs-overlay="#modal-instrumen-add" onclick="reset()">
                            <span class="sr-only">Close</span>
                            <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <div class="p-4 overflow-x-auto">
                        <form id="form-AddInstrumenModal" action="<?= site_url('/data-manajer/instrumen/add') ?>"
                            method="post">
                            <?= csrf_field() ?>
                            <input type="hidden" name="id_periode" value="<?= esc($selected_periode_id) ?>">
                            <div class="mb-4">
                                <label class="block mb-1">Nama Instrumen</label>
                                <input id="nama" type="text" name="nama" class="w-full border px-2 py-1 rounded"
                                    required>
                            </div>
                        </form>
                    </div>
                    <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-gray-700">
                        ...
                        <button type="submit" form="form-AddInstrumenModal"
                            class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                            Submit
                        </button>
                    </div>
                ...
                <div
                    ...
                    <div class="p-4 overflow-y-auto">
                        <p class="mt-1 text-gray-800 dark:text-gray-400">
                            Menghapus Instrumen ini akan menghapus data turunan di bawahnya. Apakah Anda yakin?
                        </p>
                    </div>
                    <form id="delete-form" method="post" class="inline">
                        <?= csrf_field(); ?>
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="id_instrumen" id="delete-id" value="">
                        <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-gray-700">
        ...
        <!-- Modal Re-auth -->
        <div id="modal-reauth"
            class="hs-overlay hidden fixed top-0 start-0 z-[90] w-full h-full overflow-x-hidden overflow-y-auto backdrop-blur-sm bg-black/20">
            <div
                ...
                <form action="<?= site_url('/data-manajer/instrumen/reauthDelete') ?>" method="post"
                    class="p-6 space-y-4">
                    <?= csrf_field() ?>
                    <input type="hidden" name="id_instrumen" id="reauth-id">
        ...
        <script>
            function openEditInstrumenModal(id, nama) {
                document.getElementById('instrumen-update-id').value = id;
                document.getElementById('instrumen-update-nama').value = nama;
                HSOverlay.open('#modal-instrumen-update');
            }

            function reset() {
                document.getElementById('form-UpdateInstrumenModal').reset();
                document.getElementById('form-AddInstrumenModal').reset();
            }
